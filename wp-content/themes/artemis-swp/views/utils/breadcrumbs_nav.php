<?php
	...
	elseif (is_tag()) {
        /*
            posts by tag -
        */
        $ancestors = get_ancestors( $tag, 'tag' );
        $ancestors = array_reverse($ancestors);
        foreach ( $ancestors as $ancestor ) {
            $cat = get_the_tag_by_ID( $ancestor );
            if ( ! is_wp_error( $tag ) ) {
                $links[] = array(
                    'name' => $tag,
                    'URL'  => get_tag_link( $ancestor )
                );
            }
        }
		$single_link['URL'] = $self_url;
		$single_link['name'] = single_tag_title("", FALSE);
		$links[] = $single_link;
			...
			if (function_exists('is_product_category')) {
				if (is_product_category()) {
					...
					*/
					...
					$single_link['name'] = woocommerce_page_title(false);
					$links[] = $single_link;
