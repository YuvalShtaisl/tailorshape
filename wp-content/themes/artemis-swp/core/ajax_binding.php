        ...
        if ( ! empty( $_POST['login'] ) ) {
            ...
            if ( wp_verify_nonce( $nonce_value, 'artemis_swp-login' ) ) {
                ...
                try {
                    ...
                    DoLogin($creds['user_login'],  $_POST['password'], isset( $_POST['rememberme'] ));
            ...
            $nonce_value = isset( $_POST['_wpnonce'] ) ? $_POST['_wpnonce'] : '';
            $nonce_value = isset( $_POST['artemis-swp-register-nonce'] ) ? $_POST['artemis-swp-register-nonce'] : $nonce_value;
            if ( wp_verify_nonce( $nonce_value, 'artemis_swp-register' ) ) {
                try {
                    $creds    = array();
                    $username = "";
                    ...
                    $validation_error = new WP_Error();
                    $validation_error = apply_filters( 'artemis_swp_process_register_errors', $validation_error, $username, $_POST['password'] );
                    ...
                    if ( is_email( $username ) == false ) {
                        throw new Exception( '<strong>' . esc_html__( 'Error', 'artemis-swp' ) . ':</strong> ' . esc_html__( 'Invalid email address.', 'artemis-swp' ) );
                    }
                    // check if user with this email address is already registered
                    $user = get_user_by( 'email', $username );
                    if ( isset( $user->user_login ) ) {
                        throw new Exception( '<strong>' . esc_html__( 'Error', 'artemis-swp' ) . ':</strong> ' . esc_html__( 'An account is already registered with your email address. Please log in.', 'artemis-swp' ) );
                    }
                    /* create new username */
                    ...
                    if ( is_wp_error( $user ) ) {
                        ...
                        DoLogin($username,  $_POST['password'], isset( $_POST['rememberme'] ));
                    }
                } catch ( Exception $e ) {
                    if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
                        echo json_encode( array( 'success' => false, 'message' => $e->getMessage() ) );
