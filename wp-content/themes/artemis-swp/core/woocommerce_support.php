...
if (!function_exists("ARTEMIS_SWP_woocommerce_wrapper_start")) {
...
if('grid' == ARTEMIS_SWP_get_products_view_mode() ){
    ...
    function ARTEMIS_SWP_compute_number_of_rows() {
        return ARTEMIS_SWP_get_products_per_row() * ARTEMIS_SWP_get_number_of_rows();
    }
    add_action( 'loop_shop_per_page', 'ARTEMIS_SWP_compute_number_of_rows', 100 );
    ...
    add_action('woocommerce_before_shop_loop_item', 'ARTEMIS_SWP_before_shop_loop_item', 1);
    ...
    add_action('woocommerce_shop_loop_item_title', 'ARTEMIS_SWP_shop_loop_item_title', 1);

    add_action('woocommerce_shop_loop_item_title', 'ARTEMIS_SWP_shop_loop_item_title2', 2);
    ...
    function ARTEMIS_SWP_product_list_short_description() {
        ...
        ?>
        <p class="at_product_short_description"><?php echo get_the_excerpt($product_id); ?></p>
        <?php
    }
	...
	add_action( 'woocommerce_after_shop_loop_item', 'ARTEMIS_SWP_after_shop_loop_item', 50 );
}
function ARTEMIS_SWP_before_shop_loop_item() {
    echo '<div class="at_product_image_container">';
}
function ARTEMIS_SWP_shop_loop_item_title() {
    echo "</div>";
}
function ARTEMIS_SWP_shop_loop_item_title2() {
    echo '<div class="at_product_details_container">';
}
function ARTEMIS_SWP_after_shop_loop_item() {
    echo '</div>';
}
...
function ARTEMIS_SWP_quick_view() {
    ...
    ?>
            ...
            try {
                ...
                /* In case product is actually a variation [[[ */
                $args = array(
                ...
                if ( $products->have_posts() ) {
                    ...
                    global $product;
                    global $post;
                    global $variation_product;
                    global $variation_post;
...
add_action('woocommerce_review_before_comment_text', 'woocommerce_review_display_rating', 10);
    ...
    function ARTEMIS_SWP_before_recent_products_loop( $atts ) {
        ...
        ARTEMIS_SWP_add_category_filter( $query_args, $atts['category'], 'IN');
...
function ARTEMIS_SWP_show_mobile_filter() {
    if (!ARTEMIS_SWP_is_woocommerce_active()) {
        return false;
    }
    ...
    if (is_shop() ||
        is_product_category() ||
        is_product_tag()) {
        return ARTEMIS_SWP_shop_has_sidebar() && ARTEMIS_SWP_is_mobile_toggle_sidebar_enabled();
    }

    if (is_product()) {
        /*single product page has individual sidebar setting*/
        return ARTEMIS_SWP_single_product_has_sidebar()  && ARTEMIS_SWP_is_mobile_toggle_sidebar_enabled();
    }
