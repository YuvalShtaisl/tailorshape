...
function ARTEMIS_SWP_add_settings_fields() {
	...
	$artemis_shop_settings = array(
		...
		array (
			'id'		=> 'lc_number_of_rows',
			'label'		=> esc_html__('Number of rows', 'artemis-swp'),
			'callback'	=> 'Artemis_SWP_lc_number_of_rows'
		),
		array (
		...
		array (
			'id'		=> 'lc_shop_change_shop_page_sidebar',
			'label'		=> esc_html__('Shop Pages Are Using', 'artemis-swp'),
			'callback'	=> 'Artemis_SWP_lc_shop_change_shop_page_sidebar'
		),
		array (
			'id'		=> 'lc_shop_choose_registration_password_strength',
			'label'		=> esc_html__('Choose registration password strength', 'artemis-swp'),
			'callback'	=> 'Artemis_SWP_lc_shop_choose_registration_password_strength'
		),
		array (
			'id'		=> 'lc_shop_show_mobile_toggle_sidebar',
			'label'		=> esc_html__('Enable Toggle Sidebar', 'artemis-swp'),
			'callback'	=> 'Artemis_SWP_lc_shop_show_mobile_toggle_sidebar'
		)
	);
...
function Artemis_SWP_menu_style_cbk() {
	...
	$menu_options = array(
		...
		esc_html__('Split Left&#47;Right', 'artemis-swp')	=> 'split_menu',
		esc_html__('Creative Split Left&#47;Right', 'artemis-swp')	=> 'split_menu_creative'
	);
?>
...
function Artemis_SWP_lc_number_of_rows() {
	$number_of_rows = ARTEMIS_SWP_get_theme_option( 'artemis_theme_shop_options', 'lc_number_of_rows' );

	if ( !intval( $number_of_rows ) ) {
		$number_of_rows = 4;
	}

	$rows = array(
		esc_html__( '2 Rows', 'artemis-swp' ) => '2',
		esc_html__( '3 Rows', 'artemis-swp' ) => '3',
		esc_html__( '4 Rows', 'artemis-swp' ) => '4',
		esc_html__( '5 Rows', 'artemis-swp' ) => '5',
		esc_html__( '6 Rows', 'artemis-swp' ) => '6',
		esc_html__( '7 Rows', 'artemis-swp' ) => '7',
		esc_html__( '8 Rows', 'artemis-swp' ) => '8',
		esc_html__( '9 Rows', 'artemis-swp' ) => '9',
		esc_html__( '10 Rows', 'artemis-swp' ) => '10',
	);

	?>
    <select id="lc_number_of_rows" name="artemis_theme_shop_options[lc_number_of_rows]">
		<?php ARTEMIS_SWP_render_select_options( $rows, $number_of_rows ); ?>
	</select>

    <p class="description">
		<?php echo esc_html__( 'Select number of rows in products lists (grid mode)', 'artemis-swp' ); ?>
	</p>
    <?php
}
...
function Artemis_SWP_lc_shop_change_shop_page_sidebar()
{
	$shop_change_sidebar = ARTEMIS_SWP_get_theme_option('artemis_theme_shop_options', 'lc_shop_change_shop_page_sidebar');

	if (empty($shop_change_sidebar)) {
		$shop_change_sidebar = "no";
	}

	$shop_change_sidebar_values = array(
		esc_html__('Default Sidebar', 'artemis-swp')	=> 'no',
		esc_html__('Woo Sidebar', 'artemis-swp')	=> 'yes'
	);

	?>
	<select id="lc_shop_change_shop_page_sidebar" name="artemis_theme_shop_options[lc_shop_change_shop_page_sidebar]">
		<?php ARTEMIS_SWP_render_select_options($shop_change_sidebar_values, $shop_change_sidebar); ?>
	</select>

	<p class="description">
		<?php echo esc_html__('Choose what sidebar to show on Shop pages.', 'artemis-swp'); ?>
	</p>
<?php
}

function Artemis_SWP_lc_shop_choose_registration_password_strength()
{
	$password_strength = ARTEMIS_SWP_get_theme_option('artemis_theme_shop_options', 'lc_shop_choose_registration_password_strength');

	if ( !intval( $password_strength ) ) {
		$password_strength = 4;
	}
	$password_strength_values = array(
		esc_html__('Very weak', 'artemis-swp')	=> '1',
		esc_html__('Week', 'artemis-swp')	=> '2',
		esc_html__('Medium', 'artemis-swp')	=> '3',
		esc_html__('Strong', 'artemis-swp')	=> '4'
	);

	?>
	<select id="lc_shop_choose_registration_password_strength" name="artemis_theme_shop_options[lc_shop_choose_registration_password_strength]">
		<?php ARTEMIS_SWP_render_select_options($password_strength_values, $password_strength); ?>
	</select>

	<p class="description">
		<?php echo esc_html__('Choose the strength level for the password when registering.', 'artemis-swp'); ?>
	</p>

	<?php
}

function Artemis_SWP_lc_shop_show_mobile_toggle_sidebar()
{
	 $show_mobile_toggle_sidebar = ARTEMIS_SWP_get_theme_option( 'artemis_theme_shop_options', 'lc_shop_show_mobile_toggle_sidebar' );

    if ( empty( $show_mobile_toggle_sidebar ) ) {
        $show_mobile_toggle_sidebar = "no";
    }

    $show_mobile_values = array(
        esc_html__( 'No', 'artemis-swp' )  => 'no',
        esc_html__( 'Yes', 'artemis-swp' ) => 'yes'
    );

    ?>
    <select id="lc_shop_show_mobile_toggle_sidebar" name="artemis_theme_shop_options[lc_shop_show_mobile_toggle_sidebar]">
		<?php ARTEMIS_SWP_render_select_options( $show_mobile_values, $show_mobile_toggle_sidebar ); ?>
	</select>

    <p class="description">
		<?php echo esc_html__( 'In case sidebar is enabled, in mobile show a toggle sidebar button.', 'artemis-swp' ); ?>
	</p>
<?php
}
...
?>
	...
	<p class="description">
		<?php echo esc_html__('Choose to use custom fonts for Artemis theme.', 'artemis-swp'); ?>
	</p>
