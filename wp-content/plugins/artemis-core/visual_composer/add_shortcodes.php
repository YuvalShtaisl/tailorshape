...
function ARTEMIS_SWP_reverse_letter_with_text($atts) {
	$defaults = array(
		'letter'		=> "F",
		'heading' 		=> "RlJFRSA8c3Ryb25nPlNISVBQSU5HPC9zdHJvbmc+",
		'text_content'		=> "Q29uc2VjdGV0dXIgYWRpcGlzY2luZyBlbGl0IEN1cmFiaXR1ciBzZWQgdHVycGlzIGZldWdpYXQsIG1vbGxpcyBmZWxpcyB2ZWwu"
	);
	extract(shortcode_atts($defaults, $atts));
...
function ARTEMIS_SWP_heading_with_letter($atts) {
	$defaults = array(
		'small_heading' 		=> "VGhlIGhpc3Rvcnk=",
		'big_heading'			=> "PHN0cm9uZz5BcnRlbWlzPC9zdHJvbmc+PGJyPnNpbmNlIDE5ODk=",
		'letter'				=> "A",
		"align"					=> "block_left"
	);
	extract(shortcode_atts($defaults, $atts));
...
function ARTEMIS_SWP_lc_swp_user_review_scode($atts) {
	$defaults = array(
		'reviewer_name' 	=> 'John',
		'reviewer_image'	=> '',
		'review_content' => 'Q3JlYXRpdmUgd2l0aCBjb21mb3J0IGluIG1pbmQ='
	);
	extract(shortcode_atts($defaults, $atts));
...
function ARTEMIS_SWP_at_swp_video_shortcode($atts) {
	$defaults = array(
		'video_title' 		=> 'Artemis Video',
		'bg_img' 			=> '',
		'color_overlay'		=> '',
		'video_url'			=> 'https://youtu.be/eIkN3wA5Ffg',
		'section_height'	=> '700'
	);
	extract(shortcode_atts($defaults, $atts));
	$data_bg_img = '';
	...
	if (!empty($bg_img)) {
		$data_bg_img = wp_get_attachment_url($bg_img);
	}
	else {
		$data_bg_img=vc_asset_url( 'vc/no_image.png' );
	...
	ob_start();
?>
	<div class="at_video_section <?php echo esc_attr($add_class); ?>" data-bgimage="<?php echo esc_attr($data_bg_img); ?>" data-height="<?php echo esc_attr($section_height); ?>" data-vsource ="<?php echo esc_attr($video_source); ?>" data-vid="<?php echo esc_attr($video_id); ?>">
...
function ARTEMIS_SWP_woo_best_prod_slider($atts) {
	$defaults = array(
		...
		'button_text_color'			=> 'default',
		'button_text_hover_color'	=> 'default',
		'background_text'           => 'Best products',
		'type_of_products'          => 'best',
		'product_cat'				=> ''
	);
	extract(shortcode_atts($defaults, $atts));
	...
	if( $type_of_products == 'best' )
	{
		$query_args['meta_key'] = 'total_sales';
		$query_args['orderby' ] = 'meta_value_num';
	}
	else if( $type_of_products == 'featured' )
	{
    ...
    else 
    {
        ...
        $query_args['product_cat'] = $tax_name;
    }
	...
	if ($products->have_posts()) { ?>
		<div class="at_prod_slider_container">
			<div class="at_produts_slider_inner" data-nextslide="<?php echo esc_attr($text_next); ?>" data-prevslide="<?php echo esc_attr($text_prev); ?>">
				<div class="best_products_shadow"><?php echo esc_html__($background_text); ?></div>
...
function ARTEMIS_SWP_icon_box( $atts, $content = "24/7 free shipping" ) {
	/**
	 * @var string $title
	 * @var string $add_icon
	 * @var string $icon_pos
...
function ARTEMIS_SWP_artemis_collection_promo_shortcode($atts) {
	$defaults = array(
		...
		'idle_color'			=> '#000000',
		'hover_color'			=> '#000000',
		'bold_text'				=> 'TUU8YnI+TlM=',
		'additional_text'		=> 'COLLECTION',
		'small_heading'			=> 'ACCESORIES',
		'promo_url'				=> '#'
	);
	extract(shortcode_atts($defaults, $atts));
...
?>
	<div class="partner_icon text_center transition3">
		...
		<?php } ?>
			<?php if($no_image) {?>
				<img src="<?php echo vc_asset_url( 'vc/vc-element-icons/element-icon-post-author.svg' ) ?>">
			<?php } else { ?>
...
function ARTEMIS_SWP_lc_mailchimp_subscribe_scd($atts, $content = "This form collects your name and email so that we can add you to our newsletter list for awesome news. Check our our privacy policy for more details on how we protect and manage your data.") {
	$defaults = array(
	...
	<div class="at_mc_subscr_form_container">
		<form class="at_mc_subscr_form">
			...
			<div class="consent_message_container">
				...
				<label for="swp_consent_checkbox" class="at_agree_consent_message"><?php echo wpb_js_remove_wpautop($content); ?></label>
			</div>
...
function ARTEMIS_SWP_swp_vertical_shop_promo($atts) {
	$defaults = array(
		...
		'category_name_txt'		=> 'TUU8YnI+TlM=',	/*allows <br>*/
		'link_txt'				=> 'Show All',
		'link_url'				=> '#',
		'max_height'			=> '400'
	);
	...
	if (ARTEMIS_SWP_is_woocommerce_active()) {
		...
		if($woo_sizes['width']!= '' &&  $woo_sizes['height']!= '') {
